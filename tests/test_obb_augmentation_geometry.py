...
def canonical_min_area_obb(points):
    rectangle = cv2.boxPoints(cv2.minAreaRect(points.astype(np.float32))).astype(np.float64)
    edges = np.roll(rectangle, -1, axis=0) - rectangle
    lengths = np.linalg.norm(edges, axis=1)
    index = int(np.argmax(lengths))
    angle = math.degrees(math.atan2(edges[index, 1], edges[index, 0]))
    while angle >= 90.0:
        angle -= 180.0
    while angle < -90.0:
        angle += 180.0
    long_edge = float(lengths[index])
    short_edge = float((lengths[(index - 1) % 4] + lengths[(index + 1) % 4]) / 2.0)
...
class OBBAugmentationGeometryTests(unittest.TestCase):
    ...
    def test_mosaic_boundary_geometry_is_clipped_after_corner_transform(self):
        ...
        self.assertEqual(len(output), 1)
        expected = transform_points(corners_from_target(target[0]), matrix["value"], False)
        expected[:, 0] = expected[:, 0].clip(0, 256)
        expected[:, 1] = expected[:, 1].clip(0, 256)
        expected_obb = canonical_min_area_obb(expected)
        self.assertLess(
